...
int main() {
	...
	safeCallE("create DBConnection for Logger DB", persistDB=new tDBConnection("cuLogUser", "LogPwd", "ALGO"));
	safeCallE("create Logger from persistDB connection to save results data", persistor=new tLogger(persistorDbg, persistDB));
	...
	safeCallR("train with train Set", trNN->train(trainSet));
