...
int main() {
	...
	safeCallE("create FXData for EURUSD H1", eurusdH1=new tFXData(FXDB, "EURUSD", "H1", false));
	safeCallE("create DBConnection for Persistor DB", persistDB=new tDBConnection("cuLogUser", "LogPwd", "ALGO", persistorDbg));
	safeCallE("create Logger from persistDB connection to save results data", persistor=new tLogger(persistDB, persistorDbg));
	...
	safeCallE("FX TimeSerie creation", fxTS=new TS(historyLen, FXfeaturesCnt); );
	...
	safeCallE("create train dataset from timeserie", trainSet=new DataSet(fxTS, sampleLen, predictionLen, modelFeaturesCnt, modelFeature, batchsamplesCnt_T));
	safeCallE("create run   dataset from timeserie", runSet=new DataSet(fxTS, sampleLen, predictionLen, modelFeaturesCnt, modelFeature, batchsamplesCnt_R));
...
*/
	safeCallTop("train with train Set", trNN->train(trainSet));
