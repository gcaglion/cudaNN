...
void sNN::calcErr() {
	...
	safeCallEB(Vdiff(nodesCnt[outputLevel], &F[levelFirstNode[outputLevel]], 1, u, 1, e));	// e=F[2]-u
	safeCallEB(Vssum(nodesCnt[outputLevel], e, se));										// se=ssum(e) 
...
void sNN::mallocNeurons() {
	...
	safeCallEB(myMalloc(&e, nodesCnt[outputLevel]));
	safeCallEB(myMalloc(&u, nodesCnt[outputLevel]));
}
...
void sNN::WU_std(){
	...
	safeCallEB(Vdiff(weightsCntTotal, dW, parms->LearningMomentum, dJdW, parms->LearningRate, dW));
