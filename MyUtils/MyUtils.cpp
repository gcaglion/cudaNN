...
EXPORT void _stdcall ms2ts(numtype ims, char* ots) {
...
EXPORT void			__stdcall InvertArray(int ArrLen, numtype* Arr){
	int i;
	numtype* tmpArr = (numtype*)malloc(ArrLen*sizeof(numtype));
	memcpy(tmpArr, Arr, ArrLen*sizeof(numtype));						// original array copied in temp
...
//------	Miscellaneous Utilities	- Start	----------
 EXPORT int __stdcall	FindMinMax(int pDataCount, numtype* pData, numtype* oMin, numtype* oMax){
	int i;
	numtype vMin=10000000000; numtype vMax=-10000000000;
	...
	memcpy(oMin, &vMin, sizeof(numtype));
	memcpy(oMax, &vMax, sizeof(numtype));
	//fclose(fk);
...
EXPORT void __stdcall SlideArray(int iWholeSetLen, numtype* iWholeSet, int iSampleCount, int iSampleLen, numtype** oSample, int iTargetLen, numtype** oTarget, int pWriteLog){
 ...
 EXPORT void __stdcall  DataScale(int iDataLen, numtype* iOrigData, numtype iScaleMin, numtype iScaleMax, numtype* oScaledData, numtype iScaleM, numtype iScaleP) {
...
/*EXPORT void __stdcall  DataUnScale(int iDataLen, numtype* InOutData, numtype iScaleM, numtype iScaleP){
	for (int i = 0; i < iDataLen; i++) InOutData[i] = (InOutData[i] - iScaleP) / iScaleM;
}
EXPORT void __stdcall  DataUnScale(int iDataLen, numtype* iScaledData, numtype iScaleM, numtype iScaleP, numtype* oUnScaledData){
