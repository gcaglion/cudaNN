...
typedef struct sKey : sXMLelement {
	bool find(tFileInfo* parmsFile) {
		...
		//-- restore
		if (!startTagFound) parmsFile->restorePos();
		//--
...
typedef struct sParmsSource {
	...
	EXPORT bool gotoKey(char* soughtKeyDesc, bool fromRoot=false, bool ignoreError=false) {
		...
		/*if (!fromRoot) {
			soughtKey->copyTo(tmpKey);		//-- backup sought into tmp
			currentKey->copyTo(soughtKey);	//-- copy current into sought
			tmpKey->appendTo(soughtKey);	//-- append tmp to sought
		}*/
	...
	template <typename T> EXPORT void get(T* opVal, const char* soughtParmDesc, bool fromRoot=false, bool isenum=false, int* oListLen=nullptr) {
		...
		if (fromRoot) rewind(parmsFile->handle);
		/*if(!fromRoot) {
			soughtParm->copyTo(tmpParm);		//-- backup sought into tmp
			currentParm->copyTo(soughtParm);	//-- copy current into sought
			tmpParm->appendTo(soughtParm);	//-- append tmp to sought
		}*/

		//-- set soughtKeyDepth, soughtKeyStep[][]
		if(fromRoot){
			soughtKey->depth=soughtParm->depth-1;
			for (d=0; d<soughtKey->depth; d++) {
				strcpy_s(soughtKey->step[d], XML_MAX_SECTION_DESC_LEN, soughtParm->step[d]);
			}			
		} else {
			for (d=0; d<(soughtParm->depth-1); d++) {
				strcpy_s(currentKey->step[currentKey->depth], XML_MAX_SECTION_DESC_LEN, soughtParm->step[d]);
				currentKey->depth++;
			}
		}
		
		//-- go to current key start
		if (!currentKey->find(parmsFile)) throwE("could not find key", 0);
